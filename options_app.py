...
st.markdown("**Click any row to analyze** • All features added")
...
with col_a:
    exp_filter = st.selectbox("Expiration Window", ["Nearest Expiration", "0-7 days", "7-30 days", "30-60 days", "60+ days"], key="exp_filter")
with col_b:
        ...
        with st.spinner(f"Fetching from {data_source}..."):
            try:
                if data_source == "yfinance (free, no key)":
                    ...
                    if not polygon_key:
                        st.error("Enter your Polygon API key")
                        st.stop()
